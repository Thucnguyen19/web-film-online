          ...
          <div class="row">
             <div class="col-xs-6">
                <div class="yoast_breadcrumb hidden-xs">
                    <span>PHIM TỔNG HỢP
                        @if (isset($cate_locphim))
                        <span> » <a href="">{{$cate_locphim->title}}</a></span>    
                        @endif
                        @if (isset($country_locphim))  
                        <span> » <a href="">{{$country_locphim->title}}</a></span>   
                        @endif 
                        @if (isset($genre_locphim))  
                        <span> » <a href="">{{$genre_locphim->title}}</a></span>   
                        @endif
                        @if (isset($data_year))  
                        <span> » <a href="">{{$data_year}}</a></span>   
                        @endif
                </span>
            </div>
             </div>
            ...
            <form action="{{ route('locphim') }}" method="get">
             ...
             <select class="form-select" aria-label="Default select example" name="genre" style="padding: 5px 0;width:150px; margin-right:10px">
              ...
              <select class="form-select" aria-label="Default select example" name="year" style="padding: 5px 0;width:150px; margin-right:10px">
                ...
                <option value="{{ $year }}" {{isset($data_year)  ? 'selected' : '' }}>{{ $year }}</option>                  
                @endfor
              </select>
